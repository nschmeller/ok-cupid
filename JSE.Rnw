...
\item The dataset is of modest size.  While $n = \Sexpr{n}$ is not an overwhelmingly large number of observations, the size of the data set (149.9 MB CSV file) is mostly due to the 10 essay questions.  However the sample size is large enough to have implications for many traditional statistical tests and the interpretations of the resulting p-values.  
\end{enumerate}


Other statistics education papers taking a similar approach to ours but using different data include 


We argue that given the salience of the dataset to students, this dataset provides a very fruitful channel to introduce statistical and data sciences to students of vary undergraduate levels.  

% What other papers have been published with similar types of analysis? 
% What does your dataset and analysis provide that distinguishes it from other statistics education papers?


The R code used in this \verb#.Rmd# R Sweave document can be outputed into an R script file via the \verb#purl()# command in the \verb#knitr# package.  
